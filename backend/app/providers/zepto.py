...
class ZeptoProvider(DesktopWebsiteProvider):
    ...
    async def _set_dtu_location(self, page):
        ...
        if field is None:
            ...
            field = await self._visible_first(page, self.location_inputs)
            if field is None:
                dialog_input = page.locator("[role='dialog'] input[type='text']").first
                if await dialog_input.count() and await dialog_input.is_visible():
                    field = dialog_input
        if field is None:
    ...
    async def _resolved_location_text(self, page) -> str | None:
        ...
        for selector in selectors:
            ...
            if await candidate.count() and await candidate.is_visible():
                ...
                lowered = text.lower()
                if "delhi technological university" in lowered or "delhi technological universi" in lowered or "shahbad daulatpur" in lowered or (("bawana road" in lowered or "bawana rd" in lowered) and ("dtu" in lowered or "technological university" in lowered)):
                    return text
        return None
