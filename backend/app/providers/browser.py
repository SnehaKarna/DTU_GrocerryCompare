...
class DesktopWebsiteProvider(GroceryProvider):
        ...
        if self.persistent_context:
            ...
            try:
                ...
                if system_chrome.exists():
                    try:
                        self._context = await self._playwright.chromium.launch_persistent_context(str(profile), headless=self.headless, executable_path=str(system_chrome), **context_options)
                    except Exception:
                        self._browser = await self._playwright.chromium.launch(headless=self.headless, executable_path=str(system_chrome))
    ...
    async def _verify_dtu_location(self, page) -> bool:
        ...
        return (
            ...
            or "delhi technological universi" in resolved
    ...
    async def _wait_for_cards_or_empty(self, page, card_selectors: tuple[str, ...], empty_selectors: tuple[str, ...], timeout_ms: int = 15000):
        ...
        for selector in card_selectors:
            ...
            try:
                remaining_ms = max(250, int((deadline - asyncio.get_running_loop().time()) * 1000))
                await cards.first.wait_for(state="visible", timeout=remaining_ms)
