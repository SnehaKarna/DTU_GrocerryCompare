...
class SearchService:
    def __init__(self, providers: dict[str, GroceryProvider], cache_ttl_seconds: int = settings.cache_ttl_seconds, cache: HotQueryCache | None = None, use_cache: bool = True):
    ...
    async def search(self, query: str, location: str = "DTU") -> SearchResponse:
        ...
        if self.use_cache and (blinkit_status != "unavailable" or zepto_status != "unavailable"):
...
def create_search_service() -> SearchService:
    ...
    return SearchService(providers, cache=HotQueryCache(max_entries=settings.hot_query_cache_max_entries, ttl_seconds=settings.cache_ttl_seconds), use_cache=settings.use_mock_providers)
